...
export const ProjectDetailView: React.FC<ProjectDetailViewProps> = ({ allProjects, allPosts, config }) => {
    ...
    // Parse comma-separated video URLs into an array of objects
    const parsedVideos = project.videoUrl
        ? project.videoUrl.split(',').map(item => {
            const match = item.match(/\[(.*?)\]\s*(.*)/);
            if (match) {
                return { label: match[1].trim(), url: match[2].trim() };
            }
            return { label: null, url: item.trim() };
        }).filter(v => v.url.length > 0)
        : [];
    const hasMultipleVideos = parsedVideos.length > 1;

    // Navigate to next/previous video with animation
    const handleNextVideo = (e: React.MouseEvent) => {
        e.stopPropagation();
        if (isVideoTransitioning || parsedVideos.length <= 1) return;
        setIsVideoTransitioning(true);
        setTimeout(() => {
            setCurrentVideoIndex(prev => (prev + 1) % parsedVideos.length);
    ...
    const handlePrevVideo = (e: React.MouseEvent) => {
        ...
        setIsVideoTransitioning(true);
        setTimeout(() => {
            setCurrentVideoIndex(prev => (prev - 1 + parsedVideos.length) % parsedVideos.length);
            setTimeout(() => setIsVideoTransitioning(false), 50);
        }, 300);
    };
                ...
                <div className={`fixed inset-0 z-[9999] bg-black flex items-center justify-center ${isClosing ? 'animate-view-exit' : 'animate-view-enter'}`}>
                    ...
                    <div className="w-full h-full p-4 md:p-12 md:pb-20 flex items-center justify-center">
                        <div className={`w-full max-w-7xl aspect-video relative shadow-2xl bg-black transition-opacity duration-300 ${isVideoTransitioning ? 'opacity-0' : 'opacity-100'}`}>
                            <VideoEmbed url={parsedVideos[currentVideoIndex]?.url || project.videoUrl!} autoplay={true} muted={false} />
                        </div>
                    </div>
                            ...
                            {hasMultipleVideos ? (
                                <>
                                    ...
                                    <span className={`${THEME.typography.meta} opacity-60`}>
                                        {parsedVideos[currentVideoIndex]?.label ? `${parsedVideos[currentVideoIndex].label} • ` : ''}
                                        {currentVideoIndex + 1} / {parsedVideos.length}
                                    </span>
                                    <button
            ...
            <div className={`${THEME.projectDetail.contentMaxWidth} mx-auto ${THEME.header.paddingX} py-8 md:py-16 grid grid-cols-1 md:grid-cols-12 ${THEME.projectDetail.gridGap}`}>
                ...
                <div className="md:col-span-8 space-y-24 animate-fade-in-up" style={{ animationDelay: '200ms' }}>
                    ...
                    {project.externalLinks && project.externalLinks.length > 0 && (
                        <div className="flex flex-wrap gap-4 md:gap-8 border-b border-white/10 pt-[1.5rem] pb-[2.75rem]">
